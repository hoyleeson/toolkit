 ...
 */
static int get_toa_data(struct sk_buff *skb, struct toa_data *td)
{
    struct tcphdr *th;
    int length;
    unsigned char *ptr;
...
static int tcp_option_add_label(__be32 *ptr, u32 addr, u16 port)
{
    *ptr++ = htonl((TCPOPT_TOA << 24) | TCPOLEN_TOA << 16 | port);
    *ptr++ = htonl(addr);
...
static unsigned int toa_nf_packet_setlabel(int af, struct sk_buff *skb, u32 addr, u16 port)
{
    int llen;
    struct iphdr *iph;
    struct tcphdr *th;
    ...
    llen = tcp_option_add_label((__be32 *)(th_buf + tcp_hdrlen(skb)), addr, port);
...
static unsigned int toa_nf_packet_in(int af, struct sk_buff *skb)
{
    ...
    rc = get_toa_data(skb, &td);
    if (rc) {
        return NF_ACCEPT;
    }

    printk("get toa addr:%pI4, port:%d\n", &td.ip, td.port);
...
#endif
{
    ...
    u32 addr = 0x12345678;
    u16 port = 1234;
    return toa_nf_packet_setlabel(AF_INET, skb, addr, port);
}
